...
function Settings() {
    ...
    useEffect(() => {
        chrome.storage.local.get(["defaultTab", "autoClosePanel", "showSubmissionFeedback"], (result) => {
            if (result.defaultTab) {
                setDefaultTab(result.defaultTab);
            }
            if (result.autoClosePanel !== undefined) {
                setAutoClosePanel(result.autoClosePanel);
            }
